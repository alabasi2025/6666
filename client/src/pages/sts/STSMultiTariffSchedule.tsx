...
export default function STSMultiTariffSchedule() {
  ...
  // Fetch meter info
  const { data: meterData, isLoading: isLoadingMeter } = trpc.developer.integrations.sts.meters.getInfo.useQuery(
    { meterId: meterId! },
    { enabled: !!meterId }
  );

  // Fetch tariff schedule
  const { data: scheduleData, isLoading: isLoadingSchedule, refetch: refetchSchedule } = trpc.developer.integrations.sts.tariff.getSchedule.useQuery(
    { meterId: meterId! },
    { enabled: !!meterId }
  );

  // Set tariff schedule mutation
  const setScheduleMutation = trpc.developer.integrations.sts.tariff.setSchedule.useMutation({
    onSuccess: () => {
  ...
  useEffect(() => {
    if (scheduleData?.schedule) {
      setTariffSlots(scheduleData.schedule);
    }
  }, [scheduleData]);
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-2">
            ...
            التعرفات المتعددة - عداد STS #{meterData.meterNumber}
          </h1>
            ...
            <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    أدخل وقت البداية والنهاية وسعر الكيلووات ساعة
                  </DialogDescription>
                </DialogHeader>
          ...
          {tariffSlots.length > 0 && (
            <div className="mt-4 bg-muted p-4 rounded-lg">
              ...
              <ul className="text-sm text-muted-foreground space-y-1 list-disc list-inside">
                <li>عند شحن عداد STS، سيتم حساب الكيلوهات المولدة بناءً على التعرفة الحالية للوقت</li>
