...
export default function AcrelPaymentSettings() {
  ...
  // Fetch meter info
  const { data: meterData, isLoading, refetch } = trpc.developer.integrations.acrel.meters.getInfo.useQuery(
    { meterId: meterId! },
    { enabled: !!meterId }
  );

  // Fetch payment mode
  const { data: paymentModeData, refetch: refetchPaymentMode } = trpc.developer.integrations.acrel.payment.getMode.useQuery(
    { meterId: meterId! },
    { enabled: !!meterId }
  );

  // Fetch credit info
  const { data: creditInfo, refetch: refetchCreditInfo } = trpc.developer.integrations.acrel.payment.getCreditInfo.useQuery(
    { meterId: meterId! },
    { enabled: !!meterId && paymentMode === "credit" }
  );

  // Fetch prepaid balance
  const { data: prepaidBalance, refetch: refetchPrepaidBalance } = trpc.developer.integrations.acrel.payment.getBalance.useQuery(
    { meterId: meterId! },
    { enabled: !!meterId && paymentMode === "prepaid" }
  );

  // Set payment mode mutation
  const setPaymentModeMutation = trpc.developer.integrations.acrel.payment.setMode.useMutation({
    onSuccess: () => {
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-2">
            ...
            إعدادات الدفع - عداد ACREL #{meterData.meterNumber || meterId}
          </h1>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="bg-muted p-4 rounded-lg space-y-2">
                ...
                {paymentMode === "prepaid" && (
                  <p className="text-sm text-muted-foreground">
                    الدفع المسبق: العميل يشحن رصيداً نقدياً، ويستهلك من الرصيد المدفوع.
                    <br />
                    <strong>ملاحظة:</strong> ACREL يدعم الدفع المسبق برصيد نقدي.
                  </p>
                )}
                {paymentMode === "credit" && (
                  <p className="text-sm text-muted-foreground">
        ...
        <TabsContent value="prepaid-balance">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                عرض الرصيد المتبقي للعداد (لعدادات الدفع المسبق فقط)
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {paymentMode !== "prepaid" ? (
                ...
                <>
                  {prepaidBalance && (
                    <div className="bg-muted p-6 rounded-lg space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="text-lg font-medium">الرصيد المتبقي:</span>
                        <Badge variant="default" className="text-lg px-4 py-2">
                          {prepaidBalance.balance?.toFixed(2) || "0.00"} ر.س
                        </Badge>
                      </div>
                      <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-muted-foreground">آخر مبلغ مشحون:</span>
                        <span className="text-sm">
                          {prepaidBalance.lastRechargeAmount?.toFixed(2) || "0.00"} ر.س
                        </span>
                      </div>
                    </div>
                  )}
